...
const ColorPanel = (props) => {
    ...
    const colorLable = (React.createElement("span", { className: style$2.lable },
        icon,
        React.createElement("em", { style: { backgroundColor: selectColor } })));
    return (React.createElement(Select, { disabled: disabled, className: classnames({ tooltip: !disabled }), initValue: initValue, value: currentValue, onChange: change, tooltip: lable, lable: colorLable }, areas.map(({ value }, idx) => (React.createElement(Select.Option, { className: style$2.option, key: idx }, (v, setv) => {
        return value.map((colorHex, i) => (React.createElement("span", { className: style$2.colorItem, key: `${idx}-${i}`, onMouseDown: (e) => {
                e.preventDefault();
                if (colorHex !== v) {
                    setv(colorHex);
                    setColor(colorHex);
...
const ToolBar = (props) => {
    ...
    return (React.createElement("div", { className: style$3.editorToolbar }, toolbarArea.map((toolbarArea, idx) => (React.createElement("div", { className: style$3.barArea, key: idx }, Array.isArray(toolbarArea)
