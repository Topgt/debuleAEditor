...
const MyEditor = (props) => {
    ...
    React.useEffect(() => {
        event.on('toggleInlineStyle', (style) => {
            ...
            const newState = RichUtils.toggleInlineStyle(stateRef.current, style);
            setEditorState(newState);
            return newState;
        });
        event.on('toggleBlockType', (blockType) => {
            const newState = RichUtils.toggleBlockType(stateRef.current, blockType);
            setEditorState(newState);
            return newState;
        });
    ...
    const blockRendererFn = (contentBlock) => {
        if (contentBlock.getType() === 'atomic') {
            ...
            if (type === 'IMAGE' || type === 'image') {
                return {
                    component: () => React.createElement("img", { src: "http://localhost:8000/static/code-competency-4.527bc81f.png" }),
                    editable: false,
                };
            }
            return null;
    ...
    return (React.createElement("div", { className: classnames({ formatBrush: formatBrush }, style.editor), onClick: (e) => {
        ...
        React.createElement(Editor, { customStyleMap: customStyleMap, blockRenderMap: DefaultDraftBlockRenderMap.merge(blockRenderMap), blockStyleFn: blockStyleFn, blockRendererFn: blockRendererFn, editorState: editorState, onChange: change, ref: editor => {
...
const ColorPanel = (props) => {
    ...
    const colorLable = (React.createElement("span", { className: style$1.lable },
        React.createElement("span", { className: "iconfont", dangerouslySetInnerHTML: { __html: `${fontIcon}` } }),
...
const ToolBar = (props) => {
    ...
    const renderBtn = (area, action, key) => {
        ...
        return (React.createElement("button", { key: key, disabled: disabled, className: classnames({ tooltip: !disabled }), active: active, tooltip: lable, onMouseDown: e => {
            ...
            React.createElement("div", { className: "iconfont", dangerouslySetInnerHTML: { __html: `${area.fontIcon}` } })));
    };
    const renderToolbarArea = (area, key) => {
        ...
        switch (type) {
            ...
            case 'select':
                return (React.createElement(Select, { disabled: disabled, className: classnames({ tooltip: !disabled }), key: key, onChange: (style) => event.fire(`${action}`, style), initValue: initValue, value: currentValue, tooltip: lable }, areas.map(({ fontIcon, lable = '', value }, i) => (React.createElement(Select.Option, { key: `${key}-${i}`, value: value, lable: fontIcon
                        ? React.createElement("span", { className: "iconfont", dangerouslySetInnerHTML: { __html: `${fontIcon}` } })
                    ...
                    v === value
                        ? React.createElement("span", { className: "iconfont", style: { verticalAlign: 'middle' } }, "\uE61C")
                        : '')))))));
            case 'color':
            case 'background':
                return (React.createElement(ColorPanel, { key: key, disabled: disabled, initValue: initValue, value: currentValue, change: (s) => event.fire(`${action}`, `${type}-${s}`), areas: areas, lable: lable, fontIcon: fontIcon }));
            default:
                return '';
        }
    };
    return (React.createElement("div", { className: style$2.editorToolbar }, toolbarArea.map((toolbarArea, idx) => (React.createElement("div", { className: style$2.barArea, key: idx }, Array.isArray(toolbarArea)
...
const Index = () => {
    ...
    return (React.createElement("div", { className: style.page },
